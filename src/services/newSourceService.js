...
export const getSources = async () => {
    try {
        const result = await axios.get(`${URL}source`, { headers: { 'Content-Type': 'application/json' } });
...
export const deleteSource  = async (source) => {
    try {
        const result = await axios.delete(`${URL}source?id=${source}`, { headers: { 'Content-Type': 'application/json' } });
...
export const updateSource  = async (source) => {
    ...
    try {
        const result = await axios.patch(`${URL}source?id=${source._id}`, source, { headers: { 'Content-Type': 'application/json' } });
        if (result.status === 200) {
            return { error: false, source: result.source, msg: 'Elemento guardado' };
        }
